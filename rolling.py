        ...
        for line in fin:
            ...
            if line.strip().endswith('.'):
                if current_term in line:
                    ...
                    line.replace(current_term, current_term.replace(' ', '_').replace('-', '_').replace(',', '_').replace("'", '_').replace("/", '_'))
...
def get_matrix(n=0):
    ...
    for termid1, term1 in terms:
        for termid2, term2 in terms:
            print term1, term2
            term1 = term1.replace(' ', '_').replace('-', '_').replace(',', '_').replace("'", '_').replace("/", '_')
            term2 = term2.replace(' ', '_').replace('-', '_').replace(',', '_').replace("'", '_').replace("/", '_')
